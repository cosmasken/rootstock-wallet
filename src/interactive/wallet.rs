...
async fn backup_wallet() -> Result<()> {
    ...
    let backup_path = inquire::Text::new("Enter the directory to save the backup (leave empty for current directory):")
...
async fn delete_wallet() -> Result<()> {
    ...
    let confirmed = inquire::Confirm::new(&format!("⚠️ Are you sure you want to delete wallet '{}'? This action cannot be undone.", wallet_name))
        .with_default(false)
        .prompt()?;
    
    if confirmed {
        let delete_cmd = WalletCommand {
            action: WalletAction::Delete { name: wallet_name.clone() },
        };
        
        delete_cmd.execute().await?;
        println!("\n{} {}", style("✅ Wallet deleted:").green(), style(wallet_name).bold());
    } else {

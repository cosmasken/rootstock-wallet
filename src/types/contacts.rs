...
impl Contact {
    ...
    pub fn get_transaction_history<'a>(&self, transactions: &'a [RskTransaction]) -> Vec<&'a RskTransaction> {
        transactions
            .iter()
            .filter(|tx| {
    ...
    pub fn get_recent_transactions<'a>(
        ...
        if !self.recent_transactions.is_empty() {
            let mut txs: Vec<_> = self.recent_transactions.iter()
                .filter_map(|hash| {
                    all_transactions.iter().find(|tx| tx.hash == *hash)
                })
                .collect();
        ...
        if let Some(stats) = &self.transaction_stats {
            if let Some(last_tx) = stats.last_transaction {
                if last_tx.timestamp() > chrono::Local::now().timestamp() {
                    return Err(anyhow::anyhow!("Last transaction timestamp cannot be in the future"));
                }
            }
        }
...
impl fmt::Display for Contact {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        let last_tx = self.last_transaction_time()

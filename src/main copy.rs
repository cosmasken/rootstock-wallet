...
use rootstock_wallet::registry::{load_token_registry};
...
fn _map_error(e: impl std::error::Error) -> String {
...
async fn handle_token_balance(
    ...
    _wallet: &Wallet,
    _network: &str,
    _custom_rpc: Option<&str>,
) -> Result<(), Box<dyn std::error::Error>> {
    dotenv().ok();
    let _api_key = std::env::var("ALCHEMY_API_KEY")
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
   dotenv().ok();
    env_logger::init();
    // println!("ALCHEMY_API_KEY: {:?}", std::env::var("ALCHEMY_API_KEY"));
    // println!("ALCHEMY_RPC_URL: {:?}", std::env::var("ALCHEMY_RPC_URL"));

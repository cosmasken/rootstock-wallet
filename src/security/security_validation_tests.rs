...
mod tests {
    use super::super::*;
    use crate::api::{ApiKey, ApiManager, ApiProvider};
    use crate::security::test_utils::*;
    use crate::types::contacts::Contact;
    use crate::types::wallet::{Wallet, WalletData};
    use crate::{secure_debug, secure_error, secure_info, secure_warn};
    use chrono::Local;
    ...
    fn test_wallet_debug_output_security() {
        ...
        let secure_wallet =
            Wallet::new(wallet, "Test Wallet", &password).expect("Failed to create wallet");
    ...
    fn test_log_sanitization_functions() {
        ...
        for (input, expected_markers, forbidden_content) in test_cases {
            ...
            let has_marker = expected_markers
                .iter()
                .any(|marker| sanitized.contains(marker));
            assert!(
                has_marker,
                "Sanitized message '{}' should contain one of: {:?}",
                sanitized, expected_markers
            );
    ...
    fn test_redaction_functions() {
        ...
        assert!(
            redacted_addr
                .ends_with(&mock_data.random_address()[mock_data.random_address().len() - 4..])
        );
    ...
    fn test_comprehensive_debug_output_validation() {
        ...
        let result = validator.validate_debug(
            &crate::security::redacted_debug::SecureWrapper::new(mock_wallet),
        );
    ...
    fn test_secure_logging_macros() {
        ...
        secure_debug!(
            "Test debug message with sensitive data: {}",
            mock_data.random_private_key()
        );
        secure_info!(
            "Test info message with address: {}",
            mock_data.random_address()
        );
        secure_warn!(
            "Test warn message with API key: {}",
            mock_data.random_api_key()
        );
        secure_error!(
            "Test error message with mnemonic: {}",
            mock_data.random_mnemonic()
        );
    ...
    fn test_network_security_measures() {
        ...
        assert!(
            client_result.is_ok(),
            "SecureHttpClient should be created successfully"
        );
    ...
    fn test_memory_inspector_utilities() {
        ...
        inspector.track_allocation("test_safe".to_string(), b"safe_public_data".to_vec());

        // Check for sensitive data
        let violations =
            inspector.contains_sensitive_data(&[mock_data.random_private_key().to_string()]);
        assert!(
            !violations.is_empty(),
            "Should detect sensitive data in memory"
        );
        ...
        assert!(
            ...
            "SecureString should clear memory successfully"
        );
    ...
    fn test_api_manager_security() {
        ...
        if let Some(stored_key) = api_manager.get_key(&ApiProvider::Alchemy, "mainnet") {
            ...
            assert!(
                key_debug.contains("...") || key_debug.contains(&mock_data.random_api_key()[..3])
            );
        }

...
//             .sign_transaction(tx)
//             .await
//             .map_err(|e| WalletError::SerializationError(e.into()))?;
//         Ok(signed_tx)
//     }
...
//         let keystore = eth_keystore::encrypt_key(
//             dir.path(),
//             &mut OsRng,
//             &private_key,
//             password,
...
//         password: &str,
//     ) -> Result<Self, WalletError> {
//         let keystore_file = File::open(keystore_path)?;
//         let keystore: eth_keystore::Key = serde_json::from_reader(BufReader::new(keystore_file))?;
//         let private_key = eth_keystore::decrypt_key(&keystore, password)?;
...
//         manager
//             .set_current_wallet(&wallet2.address)
//             .unwrap();
//         assert_eq!(
...
use ethers::types::{transaction::eip2718::TypedTransaction, Bytes};
use eth_keystore::{self, KeystoreError};
use k256::ecdsa::SigningKey;
use log::{error, info};
use rand::rngs::OsRng;
